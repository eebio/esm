...
file=load("/home/micha/EEBio/example/20191121_FLOPR_EXAMPLE/20191121_beads.fcs")
...
# print(x)
maxr=1048576
dat_mask=[0 < xi < maxr && 0 < yi < maxr && 0 < zi < maxr for (xi, yi,zi) in zip(x, y,z)]
# print(dat_mask)
...
hist_counts = fit(Histogram, (x, y); nbins=2000)  # Compute histogram
...
threshold = 0.01  # Define a density threshold
...
fraction_to_keep = 0.85  # Keep top 20% of highest density points
...
# scatter(x_top, y_top, color=:blue, label="Points inside KDE contour (top 20%)", alpha=0.9, title="Filtered Points Inside KDE Contour")
xlabel!("x")
ylabel!("y")

histogram(z[top_indices],bins=hist_bins[1],xaxis=(:log10,(1e+0, 1e+10)))

# Optional: Overlay KDE contour for reference
# x_grid = 0:10000:1e+7
# y_grid = 0:10000:1e+7
# kde_grid_values = [pdf(kd, xi, yi) for xi in x_grid, yi in y_grid]  # KDE over a grid
# contour!(x_grid, y_grid, kde_grid_values, levels=[threshold], color=:red, linewidth=2, label="KDE Contour")

# Step 10: Plot the points outside the histogram bins
# scatter!(x_outside, y_outside, color=:orange, label="Points outside histogram bins", alpha=0.4)
plot!(xscale=:log10, minorgrid=true,legend=false)
xlims!(1e+0, 1e+10)
# ylims!(1e+0, 1e+10)
